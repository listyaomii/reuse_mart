...
class ProdukClient {
  static const String baseUrl =
      'https://api2.reuse-mart.com/api'; // Untuk emulator Android
  ...
  Future<List<Produk>> fetchProducts() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final filteredData =
            data.where((json) => json['status_produk'] == 'tersedia').toList();
  ...
  Future<Produk> fetchProductDetail(String productId) async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/produk/$productId'))
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Gagal memuat detail produk: ${response.statusCode} - ${response.body}');
      }
    } catch (e) {
  ...
  Future<List<Kategori>> fetchCategories() async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/kategori'))
          .timeout(Duration(seconds: 10));
